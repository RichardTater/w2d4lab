...
// CODE HERE
const totalPopulation = populations.reduce((accumulative, current) => accumulative + current)
...
const ordersWithTax = orders.map((element) => element.price * (1 + element.tax))
...
const bobsTotal = purchases.filter((element) => element.owner === "Bob").reduce((accumulative, current) => accumulative + current.price, 0)
